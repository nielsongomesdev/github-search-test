...
import { Box, Flex, Spinner, Text, Grid, GridItem, Image, Button, VStack, Select, HStack, Wrap } from "@chakra-ui/react";
          ...
          <GridItem>
            <Flex direction="column" gap={6}>
              <Flex direction="column" gap={6} bg="white" p={6} borderRadius="4px" boxShadow="sm" border="1px solid" borderColor="gray.200">
                <Flex direction="column" gap={4} align="start" display={{ base: "flex", md: "none" }} w="100%">
                  <HStack spacing={4} align="flex-start" w="100%">
                    <Image src={user.avatar_url} alt={user.name || user.login} borderRadius="full" boxSize="56px" objectFit="cover" border="1px solid" borderColor="border.gray" flexShrink={0} />
                    <Box flex={1}>
                      <Text fontSize="lg" fontWeight="700" color="gray.800" lineHeight="1.2">{user.name || user.login}</Text>
                      <Text fontSize="14px" fontWeight="400" lineHeight="150%" color="rgba(160, 174, 192, 1)">@{user.login}</Text>
                    </Box>
                  </HStack>

                  <HStack spacing={6} fontSize="sm" color="gray.600" w="100%">
                    <Flex gap={1} alignItems="center">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
                      <Text fontWeight="600" color="gray.800">{user.followers}</Text>
                      <Text fontSize="xs">{t('followers')}</Text>
                    </Flex>
                    <Flex gap={1} alignItems="center">
                      ...
                      <Text fontWeight="600" color="gray.800">{user.following}</Text>
                      <Text fontSize="xs">{t('following')}</Text>
                    </Flex>
                  </HStack>

                  {user.bio && <Text color="gray.600" fontSize="sm" lineHeight="1.4" textAlign="left">{user.bio}</Text>}

                  <Wrap spacing={2} w="100%">
                    {user.location && <Flex gap={1} alignItems="center" px={2} py={1} bg="gray.100" borderRadius="2px"><Text fontSize="xs">📍 {user.location}</Text></Flex>}
                    {user.email && <Flex gap={1} alignItems="center" px={2} py={1} bg="gray.100" borderRadius="2px"><Text fontSize="xs">📧 {user.email}</Text></Flex>}
                    {user.company && <Flex gap={1} alignItems="center" px={2} py={1} bg="gray.100" borderRadius="2px"><Text fontSize="xs">🏢 {user.company}</Text></Flex>}
                  </Wrap>
                  ...
                  <Wrap spacing={2} w="100%">
                    {user.blog && (
                      <Button
                        ...
                        fontWeight="normal"
                        fontSize="xs"
                        leftIcon={<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg>}
                      >
                ...
                <Flex gap={4} alignItems="center" display={{ base: "none", md: "flex" }}>
                  <Image src={user.avatar_url} alt={user.name || user.login} borderRadius="full" boxSize="48px" objectFit="cover" border="1px solid" borderColor="border.gray" flexShrink={0} />
                  <Box flex={1}>
                    <Text fontSize="lg" fontWeight="700" color="gray.800" lineHeight="1.2">{user.name || user.login}</Text>
                    <Text fontSize="14px" fontWeight="400" lineHeight="150%" color="rgba(160, 174, 192, 1)">@{user.login}</Text>
                  </Box>
                ...
                <VStack align="start" spacing={3} color="gray.600" fontSize="sm" w="100%" display={{ base: "none", md: "flex" }}>
                  ...
                  {user.email && <Flex gap={2} alignItems="center"><Text>📧 {user.email}</Text></Flex>}
                  {user.blog && (
                    <Button
                      as="a"
                      href={user.blog.startsWith('http') ? user.blog : `https://${user.blog}`}
